...
class Text2Image(Plugin):
    ...
    def load_config(self):
        ...
        try:
            ...
            return {
                ...
                "blacklist_keywords": [],
                "whitelist_keywords": [],
                "emoji_display_mode": "text"  # emoji显示模式：native(原生)、text(文本)、description(描述)
            }
    ...
    def process_emoji(self, text):
        """处理文本中的表情符号，根据配置决定显示方式"""
        emoji_mode = self.config.get("emoji_display_mode", "text")
        
        # 微信表情映射（用于description模式）
        wx_emoji_map = {
            "[微笑]": "(微笑)",
            "[撇嘴]": "(撇嘴)",
            "[色]": "(色)",
        ...
        if emoji_mode == "native":
            # 转换为原生emoji
            for wx_emoji, unicode_emoji in emoji_map.items():
                text = text.replace(wx_emoji, unicode_emoji)
        elif emoji_mode == "description":
            # 转换为文字描述
            for wx_emoji, desc in wx_emoji_map.items():
                text = text.replace(wx_emoji, desc)
        # 对于text模式，不做任何处理，保留原始的[表情]格式
    ...
    def text_to_image(self, text):
        ...
        try:
            ...
            # 处理表情符号
            formatted_text = self.process_emoji(formatted_text)
